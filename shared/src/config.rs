...
impl Config {
    ...
    pub fn config_path_candidates() -> Vec<PathBuf> {
        ...
        if let Ok(xdg_config_home) = std::env::var("XDG_CONFIG_HOME") {
            paths.push(PathBuf::from(xdg_config_home).join("climonitor").join("config.toml"));
        } else if let Some(home_dir) = home::home_dir() {
            paths.push(home_dir.join(".config").join("climonitor").join("config.toml"));
        }
...
"#;
    ...
    fn test_config_file_operations() {
        ...
        assert_eq!(loaded_config.connection.tcp_bind_addr, config.connection.tcp_bind_addr);
